...
import { McpHubServer } from "./mcp-hub-server.js";
import { 
...
// Create roots manager instance
const rootsManager = new RootsManager();

// Create MCP server manager instance (auto load enabled)
const serverManager = new McpServerManager({
  autoLoad: true,
  rootsManager: rootsManager, // Pass roots manager to server manager
});

// Create custom MCP Hub server with roots support
...
server.tool(
  "call-tool",
  "Call a specific tool from a specific server. TIP: Use find-tools first to discover the tool and get the correct serverName and toolName. Respects client-defined roots boundaries.",
  {
  ...
  async (args, extra) => {
    try {
      ...
      // Check for path arguments and validate against roots if supported
      if (rootsManager.supportsRoots && toolArgs) {
        const pathArgs = ['path', 'file', 'filepath', 'filename', 'directory', 'folder'];
        
        for (const [key, value] of Object.entries(toolArgs)) {
          if (typeof value === 'string' && pathArgs.some(pathArg => 
            key.toLowerCase().includes(pathArg) || 
            (typeof value === 'string' && (value.startsWith('/') || value.includes('file://')))
          )) {
            if (!rootsManager.isPathInRoots(value)) {
              return {
                content: [
                  {
                    type: "text",
                    text: `Access denied: Path '${value}' is outside the allowed roots boundaries. Current roots: ${JSON.stringify(rootsManager.roots.map(r => r.uri), null, 2)}`,
                  },
                ],
                isError: true,
              };
...
// Tool to get current roots information
server.tool(
  "get-roots",
    ...
    try {
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify({
              supportsRoots: sessionState.supportsRoots,
              rootsListChanged: sessionState.rootsListChanged,
              rootsCount: sessionState.roots.length,
              roots: sessionState.roots,
              filePaths: rootsManager.getFilePathsFromRoots()
            }, null, 2),
          },
        ],
      };
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error getting roots information: ${(error as Error).message}`,
          },
        ],
      };
    }
